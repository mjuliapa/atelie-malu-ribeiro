      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-brand-cream border-b border-brand-line">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-1 absolute right-4">
            <a href={'https://wa.me/' + MALU_WHATSAPP} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-white transition-colors" style={{ color: '#25D366' }}>
            ...
            <button onClick={handleLogout} className="p-2 rounded-lg text-brand-muted hover:text-brand-text hover:bg-white transition-colors">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-5 h-5">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-brand-line">
        <div className="flex items-center justify-around px-1 py-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn('flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-colors', isActive ? 'text-brand-mauve' : 'text-brand-muted hover:text-brand-text')}>
                {item.icon}
                <span className={cn('text-[10px] font-medium', isActive && 'text-brand-mauve')}>{item.label}</span>
              </Link>
            )
          })}
        </div>
