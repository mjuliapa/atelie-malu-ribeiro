    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl px-6 py-6 z-10">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onClick={onClose}
            className="p-2 text-brand-muted hover:text-brand-text rounded-lg hover:bg-brand-cream transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border border-brand-line bg-brand-cream text-brand-text focus:outline-none focus:border-brand-mauve focus:ring-1 focus:ring-brand-mauve transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setMaxStudents((n) => Math.max(1, n - 1))}
              ...
              <span className="text-2xl font-display text-brand-text w-8 text-center">
                {maxStudents}
              </span>
              <button
                type="button"
                onClick={() => setMaxStudents((n) => Math.min(20, n + 1))}
                className="w-10 h-10 rounded-lg border border-brand-line bg-white text-brand-text hover:bg-brand-cream transition-colors font-bold text-lg flex items-center justify-center"
              >
                +
              </button>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 px-4 bg-brand-ink text-brand-cream rounded-lg font-medium text-sm hover:bg-brand-text transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
