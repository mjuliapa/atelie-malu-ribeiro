...
export function ArgilaRowActions({ id, currentValue }: { id: string; currentValue: number }) {
  const router = useRouter()
  const [busy, setBusy] = useState(false)

  async function handleEdit() {
    const novo = window.prompt('Novo valor total (R$):', String(currentValue))
    if (novo === null) return
    const valor = parseFloat(novo.replace(',', '.'))
    if (isNaN(valor) || valor <= 0) return alert('Valor inválido.')
    setBusy(true)
    const res = await fetch(`/api/admin/argila?id=${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ total_value: valor }),
    })
    setBusy(false)
  ...
  async function handleDelete() {
    if (!window.confirm('Excluir esta venda de argila?')) return
    setBusy(true)
