...
export default async function AdminDashboardPage() {
  ...
  // Peças em aberto
  const { count: openPieces } = await supabase
    .from('pieces')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'open')

  // Valor total em aberto
  const { data: openValue } = await supabase
    .from('pieces')
  ...
  // Aulas de hoje
  const { count: todaySlots } = await supabase
    .from('schedule_slots')
    .select('*', { count: 'exact', head: true })
    ...
    <>
      ...
      <div className="px-4 pt-4 pb-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link href="/admin/pecas" className="bg-white rounded-xl p-4 shadow-card hover:shadow-card-hover transition-shadow">
          ...
          <Link href="/admin/fechamentos" className="bg-brand-blush rounded-xl p-4 shadow-card hover:shadow-card-hover transition-shadow">
            <p className="text-xs text-brand-mauve mb-1">A receber</p>
            <p className="font-display text-2xl text-brand-mauve">{formatCurrency(totalOpen)}</p>
          </Link>

          <Link href="/admin/agenda" className="bg-white rounded-xl p-4 shadow-card hover:shadow-card-hover transition-shadow">
            <p className="text-xs text-brand-muted mb-1">Aulas hoje</p>
            <p className="font-display text-3xl text-brand-text">{todaySlots ?? 0}</p>
          </Link>

          <Link href="/admin/alunos" className="bg-white rounded-xl p-4 shadow-card hover:shadow-card-hover transition-shadow">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link href="/admin/pecas/nova"
              className="flex items-center gap-2 bg-brand-ink text-brand-cream px-4 py-3 rounded-xl text-sm font-medium hover:bg-brand-text transition-colors">
              <span>+</span> Nova peça
            </Link>
            <Link href="/admin/fechamentos/novo"
              className="flex items-center gap-2 bg-white text-brand-text border border-brand-line px-4 py-3 rounded-xl text-sm font-medium hover:bg-brand-cream transition-colors">
        ...
        {recentPieces && recentPieces.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/pecas" className="text-xs text-brand-mauve hover:underline">
                Ver todas
              </Link>
            </div>
                ...
                <div key={piece.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-brand-text">
                      {formatCurrency(piece.calculated_value)}
                    </p>
